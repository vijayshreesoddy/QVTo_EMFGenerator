...
import com.uni.de.qvto.profiler.views.QVToProfilerTreeTableView;
import com.uni.de.qvto.profiler.views.QVToProfilerView;
...
public class UML2RDBHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		//ShellExecutor.executeShellScript();
	    //ProfilierModelProvider.INSTANCE.getProfilierList().clear();
		//ModelCreater.createModel();
		QVToDialog dialog = new QVToDialog(Display.getDefault().getActiveShell());
		 if (dialog.open() == Window.OK) {
	            String configPath = dialog.getConfigFilePath();
	            if(dialog.isAutoGenerate()) {
		    		QVToCommandLine.executeQVToScripts(configPath, dialog.numberofModelElements());
	            } else {
		    		QVToCommandLine.executeQVToScripts(configPath);
	            }

	        }
		// Refer to an existing transformation via URI
		/*URI transformationURI = URI.createFileURI("/Users/vijayshree/QVTo_BB_04_02_2021/com.uni.de.qvto.uml2rdb/Simpleuml_To_Rdb.qvto");
        ...
        QVToProfilerTreeTableView view = (QVToProfilerTreeTableView) page.findView(QVToProfilerTreeTableView.ID);
        view.getViewer().setInput(ProfilierModelProvider.INSTANCE.getTraceModel());
        view.addBarChart();
