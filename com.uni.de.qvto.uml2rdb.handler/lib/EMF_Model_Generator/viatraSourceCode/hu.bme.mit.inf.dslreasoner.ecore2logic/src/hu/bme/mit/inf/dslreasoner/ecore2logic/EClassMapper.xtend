...
interface EClassMapper {
	def void transformEClasses(Ecore2Logic_Trace trace, LogicProblem problem, Iterable<EClass> classes,
		int numberOfObjects, EcoreMetamodelDescriptor metamodelDescriptor)
...
class EClassMapper_AllElementAsObject implements EClassMapper {
	...
	override transformEClasses(
		Ecore2Logic_Trace trace,
		LogicProblem problem,
		Iterable<EClass> classes,
		int numberOfObjects,
		EcoreMetamodelDescriptor metamodelDescriptor
	) {
...
//				Supertype(classMapperTrace.typeMap.get(c),classMapperTrace.typeMap.get(s))
				if(classMapperTrace.typeMap.containsKey(s)) Supertype(classMapperTrace.typeMap.get(c),
					classMapperTrace.typeMap.get(s))
			}
		}
	}
